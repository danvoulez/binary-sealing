"""Tests for diamond_manifest_builder."""
...
class TestDiamondManifestBuilder(unittest.TestCase):
    def _seed(self, nas_root: str):
        ...
        return projection_result, sealability_result, context

    def test_build_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            projection_result, sealability_result, context = self._seed(tmp)
            diamond_builder = DiamondManifestBuilder(nas_root=tmp, receipts=NullReceipts())
            ...
            self.assertTrue(os.path.exists(candidate.manifest_path))
            self.assertTrue(candidate.diamond_candidate_id.startswith("diamond-candidate:"))
