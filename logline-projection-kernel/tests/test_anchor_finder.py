...
class TestAnchorFinder(unittest.TestCase):
    def _seed(self, nas_root: str):
        ...
        builder.build(lens=lens, actor="x", scope={"project": "test"})
        return cerebro_root
    ...
    def test_find_selected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            request = AnchorRequest(
                actor="x",
                task="Explain LogLine",
                purpose="Grounding",
                query_concepts=["logline"],
            )
            result = finder.find(request)
            self.assertEqual(result.anchor_state, "selected")
            self.assertTrue(os.path.exists(result.anchor_path))
