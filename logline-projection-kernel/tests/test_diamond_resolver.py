...
class TestDiamondResolver(unittest.TestCase):
    def _sealed(self, nas_root: str, this_value: str = "gdoc:doc@1"):
        fixture_path = os.path.join(nas_root, "doc.txt")
        with open(fixture_path, "w", encoding="utf-8") as f:
            f.write("# LogLine\n\nThis document describes Cerebro, Flux, Diamond and operator mesh.\n")

        act = Act(
            who="x", did="y", this=this_value, when="2026-07-02T12:00:00Z",
            status="registered",
    ...
    def test_resolve_current(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._sealed(tmp, "gdoc:doc@1")
            self._sealed(tmp, "gdoc:doc@2")
            resolver = DiamondResolver(nas_root=tmp, receipts=NullReceipts())
            request = DiamondResolveRequest(
                actor="x",
                task="Explain",
                purpose="Grounding",
                query_concepts=["logline"],
                intended_access_mode="anchor",
            )
            result = resolver.resolve(request)
            self.assertEqual(result.resolution_state, "resolved_to_current_diamond")
            self.assertTrue(os.path.exists(result.resolution_path))
            self.assertTrue(result.selected_diamond_id.startswith("diamond:"))
