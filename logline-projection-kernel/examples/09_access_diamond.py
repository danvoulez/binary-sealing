"""Example: request access to a sealed Diamond."""
...
from logline_kernel.diamonds.access.gate import AccessActor, AccessRequest, DiamondAccessGate
...
def build_diamond(nas_root: str):
    ...
    return sealer.seal_candidate(
        ...
        authority=authority,
        actor="op:lab-256",
        seal_note="Seal for access example.",
    )


def main():
    nas_root = tempfile.mkdtemp(prefix="logline_nas_")
    seal_result = build_diamond(nas_root)

    gate = DiamondAccessGate(nas_root=nas_root, receipts=NullReceipts())
    request = AccessRequest(
        actor=AccessActor(actor_id="op:cloud", roles=["operator"]),
        mode="anchor",
        purpose="agent_grounding",
        reason="Operator needs a safe anchor before explaining the LogLine product spine.",
        scope={"conversation": "current", "allowed_use": "reasoning_inside_boundary"},
    )
    result = gate.request_access(
        sealed_manifest_path=seal_result.sealed_manifest_path,
        request=request,
        actor="op:cloud",
    )

    print("allowed", result.allowed)
    print("request_hash", result.request_hash)
    print("package_path", result.package_path)
