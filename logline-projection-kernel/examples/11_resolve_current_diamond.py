"""Example: resolve the current Diamond for a task."""
...
def main():
    ...
    build_and_seal_diamond(nas_root, "gdoc:projection-kernel-pack@rev42")
    build_and_seal_diamond(nas_root, "gdoc:projection-kernel-pack@rev43")

    resolver = DiamondResolver(nas_root=nas_root, receipts=NullReceipts())
    request = DiamondResolveRequest(
        actor="op:cloud",
        task="Explain the LogLine product spine without drifting.",
        purpose="Find the current sealed Diamond that can anchor reasoning.",
        query_concepts=["projection_kernel", "dynamic_projection", "flux", "cerebro", "diamond", "operator_mesh"],
        intended_access_mode="anchor",
        max_doubt_severity=DOUBT_SEVERITY["missing"],
        scope={"actor_roles": ["operator"]},
    )
    result = resolver.resolve(request)

    print("resolution_state", result.resolution_state)
    print("selected_diamond_id", result.selected_diamond_id)
    print("resolution_path", result.resolution_path)
